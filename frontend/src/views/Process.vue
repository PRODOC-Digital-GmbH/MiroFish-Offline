<template>
  <div class="process-page">
    <!-- Top navigation bar -->
    <nav class="navbar">
      <div class="nav-brand" @click="goHome">SIMULAB</div>
      
      <!-- Center step indicator -->
      <div class="nav-center">
        <div class="step-badge">STEP 01</div>
        <div class="step-name">{{ $t('process.buildProcess') }}</div>
      </div>
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          ...
          <div v-else-if="currentPhase === 1 && !graphData" class="graph-waiting">
            ...
            <p class="waiting-hint">{{ $t('process.buildingGraphHint') }}</p>
          </div>
      ...
      <div class="right-panel" :class="{ 'hidden': isFullScreen }">
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              <div class="detail-section">
                <div class="detail-label">{{ $t('process.ontologyDesc') }}</div>
              </div>
              ...
              <div class="detail-section" v-if="projectData?.ontology">
                ...
                <div class="relation-list">
                  ...
                  <div v-if="(projectData.ontology.relation_types?.length || 0) > 5" class="relation-more">
                    {{ $t('process.moreRelationships', { count: projectData.ontology.relation_types.length - 5 }) }}
                  </div>
                </div>
              </div>
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.waitingOntologyGeneration') }}</div>
              </div>
            </div>
          </div>
        ...
        <div class="project-panel">
          <div class="project-header">
            ...
            <span class="project-title">{{ $t('process.projectInformation') }}</span>
          </div>
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = t('process.noFilesPending')
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.failedToLoadProject')
    }
  } catch (err) {
    console.error('Load project error:', err)
    error.value = t('process.failedToLoadProject') + ': ' + (err.message || '')
  } finally {
    loading.value = false
  }
}
...
const startBuildGraph = async () => {
  try {
    ...
    if (response.success) {
      ...
      error.value = response.error || t('process.failedToStartBuild')
      buildProgress.value = null
    }
  } catch (err) {
    console.error('Build graph error:', err)
    error.value = t('process.failedToStartBuild') + ': ' + (err.message || '')
    buildProgress.value = null
  }
}
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        stopPolling()
        stopGraphPolling()
        error.value = t('process.graphBuildFailed') + ' ' + (task.error || '')
        buildProgress.value = null
      }
    }
  } catch (err) {
