...
3. [Language Consistency - ALWAYS Write in {output_language}]
   - The entire report MUST be written in {output_language}, regardless of source material language
   - Tool-returned content may contain other languages
   - When quoting tool-returned content in a different language, ALWAYS translate it to fluent {output_language} before writing to report
...
- ALWAYS respond in {output_language}, regardless of the language used in source material or report content"""
...
class ReportAgent:
    ...
    def plan_outline(
        ...
        system_prompt = PLAN_SYSTEM_PROMPT.format(output_language=Config.OUTPUT_LANGUAGE)
